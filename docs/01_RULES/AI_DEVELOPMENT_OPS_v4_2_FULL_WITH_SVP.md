...
### 11.3 Prop-Drilling Verification Protocol（プロップ伝達検証プロトコル）
バグ再発防止のため、コールバック関数や重要なステートを子コンポーネントに渡す際は以下を検証すること：
1. **全レンダーパスの確認**: `if (mode === 'A') ... else ...` のような条件分岐がある場合、**全ての分岐**でプロップが正しく渡されているか確認する。
2. **End-to-End Trace**: 親コンポーネント（定義元）から末端の孫コンポーネント（使用元）まで、プロップ名で `grep` し、鎖が切れていないか確認する。
3. **実効アクションの検証**: ブラウザテストにおいて、「ボタンが押せること」だけでなく「期待される副作用（DB更新、画面遷移など）が発生したこと」をログやDOM変化で確認する。
