...
## 4. Life × Execution 関係性（衝突ルール改定）
*   **相互不干渉**: Life（生活）とExecution（仕事）は独立して進行する。
*   **前提条件ではない**: Lifeが未完了でもExecutionは開始できる（「朝仕事→夜家事」を許容）。
*   **役割**: LifeはExecutionが進まなかった日の「正当な理由（文脈）」として機能する。
    *   Executionあり = 仕事を進めた日。
    *   Executionなし + Lifeあり = 生活を整えた日（正当な一日）。
*   **非懲罰**: ExecutionもLifeも進まなかったとしても、事実のみを記録し、評価・警告は行わない。
