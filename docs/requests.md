...
- **【中】R-150** ガントチャートでマウスオーバー中の行を、左の見出し列まで含めて分かりやすく強調（下線を少し太く）
  - 2026-08-19 会話原文: 「ガントチャートでマウスオーバーしている行がほかよりちょっとだけ下線が太くなるってできる？というのも、マウスオーバーしている行が左の見出し表のどれにあったってるのかわかりづらいんだよ」
  - **→ R-150 として requests_log.md に移記済み（2026-08-19）**

- **【中】** UIレビュー: 見た目が同じ表示なのに文脈（絞り込み・表示モード・スコープ）で意味・分母・分子が変わる箇所を洗い出し、UIで区別できるようにする
  - 2026-08-19 会話原文（R-146「プロジェクト別のガント合計＝絞り込み中はそのプロジェクトの合計」を受けて）: 「でも、そういう、表示してる見た目は同じなのに意味合いや中身分母分子が違うとかは、UIでわかるようにすべきだよな、全体をcodexでレビューしてほしいその点について」
  - 2026-08-19 Codex レビュー完了: `docs/reference/review/2026-08-19-同形異義UIレビュー_codex.md`（9件: 高5・中2ほか）。指揮AI分割案: R-147（バグ寄り: 週負荷の分母がスコープ無視／超過分パネルの編集欄が `due_date` 固定で有効締切と不一致）→ R-148（「集計値の隣に母集団1語」ルールを一括適用）→ R-149（時間丸めの統一、後回し）
  - 2026-08-19 発注者回答原文: 「UIのデザインがくずれるかな？心配だけどやってみてほしい」
  - **→ R-147・R-148 として requests_log.md に移記済み（2026-08-19）。R-149（丸め統一）は下記に残置**
