...
- **【高】2026-07-07 量感カレンダー/ガント改善**
  - ガントチャートに、完了済み・削除済み・過去のアイテムなど、表示すべきでないアイテムが表示されている可能性がある。現状の表示条件を整理し、表示されるべきかどうかを判断したうえで修正する。
  - ガントチャートのプロジェクト別表示で、左列のアイテムタイトル右クリックメニューに「前に挿入」「後に挿入」を追加する。選択したアイテムの前後にインライン入力で新規項目を追加できるようにする。追加項目は右クリックされたアイテムと同じテナント・同じプロジェクトに所属する。
  - 量感カレンダーのグリッド表示と、詳細画面カレンダーのスクロール動作が遅い。原因を調査し、スクロール性能を改善する。
