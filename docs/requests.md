...
- **【中】** ガントチャート日付ヘッダーに合計時間数を表示
  - 2026-08-13 会話内発言原文: 「ガントチャートで縦（列）が日付だけど、その日付の下に合計何時間になっているのかを表示する機能をつけて。その日のキャパ割合を色で表すバーが存在するから、その下に時間数を書こうか。単位は時間だ。」
  - 指揮AI事前調査済み: `RyokanGanttView.tsx`の日付ヘッダー列（590〜632行目付近）は幅`w-6`(24px)の狭い列で、曜日・日付・`CapacityBar`（R-034実装、4px高さのキャパ割合バー）が縦に並んでいる。`stats.total`（分単位の合計割当時間）は既に`dailyCapacityStats`から取得済みでCapacityBarに渡されている。CapacityBarの直下に時間換算（分→時間、小数点表示要検討）のテキストを追加する想定
