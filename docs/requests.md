...
- **【高】R-145** バグ: ガント時間軸で 24h をはみ出すブロックが幅 1 分の極小表示になる（F-40「右端揃え」の実装取り違え）
  - 2026-08-19 会話原文（ガントのスクリーンショット添付）: 「この板１上下加工、板２，板３上下加工　の予定がガントチャート上で幅がすごく小さく表示されてるのはまちがいだよね？」
  - 調査Agent結論（2026-08-19、読み取り専用）: 仕様 `03_画面設計.md` F-40「24hはみ出し時はブロックの右端を24:00に揃えて描画（幅は保つ）」に対し、実装 `logic/ganttTimeBlocks.ts:44-52` は開始位置を据え置いて幅を 24:00 で切り落としている。板２・板幅決めは手動オフセットのドラッグ飽和値（1439分）が保存されており幅1分、板３は依存カスケードで前工程の終端 1440 起点→クランプで幅1分。`RyokanGanttView.timelineBlocks.test.tsx:181` はテスト名「右端が日末尾に揃い」に対しアサーションが切り詰めを検証しており取り違えが固定化。status は無関係（現在は全て focus）。板１は `prep_date` が null のためそもそも時間軸に描かれない（別件候補: due だけのタスクがガント時間軸に出ない）。副次: `meta.gantt_time_blocks` に日付移動後の孤児キーが残る（軽微）
  - 修正案: `ganttTimeBlocks.ts` 5行（はみ出す場合 `start = DAY_MINUTES − allocated`、幅は保持、`overflow=true`）＋テスト3件を仕様に合わせる＋回帰ケース「先行がはみ出しても後続は1分幅にならない」
  - 発注者回答（2026-08-19）: 「バグ修正進めてr-145」
  - **→ R-145 として requests_log.md に移記済み（2026-08-19）**
