...
export const DecisionDetailModal: React.FC<DecisionDetailModalProps> = ({ item, onClose, onDecision, onDelete }) => {
    if (!item) return null;
                        ...
                        <div className="px-6 py-2 space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-100 dark:border-slate-800">
                                    <div className="flex-1">
                                        ...
                                        {item.due_status === 'waiting_external' ? (
                                            ...
                                            <input
                                                ...
                                                onChange={async (e) => {
                                                    await ApiClient.updateItem(item.id, { due_date: e.target.value, due_status: 'confirmed' });
                                                }}
                                    ...
                                    <button
                                        onClick={async () => {
                                            const newStatus = item.due_status === 'waiting_external' ? 'confirmed' : 'waiting_external';
                                            const newDate = newStatus === 'waiting_external' ? null : (item.due_date || null);
                                            await ApiClient.updateItem(item.id, { due_status: newStatus, due_date: newDate });
                                            // Ideally trigger refresh, but optimistically assumed
                                        }}
                                        className="text-xs text-slate-400 underline hover:text-indigo-500 whitespace-nowrap"
                                    >
                                        {item.due_status === 'waiting_external' ? '[日付を入力する]' : '[未確定にする]'}
                                    </button>
                                </div>
                            </div>
