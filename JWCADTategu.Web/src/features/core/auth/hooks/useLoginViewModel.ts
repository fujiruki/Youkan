...
export const useLoginViewModel = () => {
	...
	const login = async (creds: LoginCredentials, accountType: 'user' | 'tenant' = 'user') => {
		...
		try {
			...
			if (res.tenant) {
				localStorage.setItem(YOUKAN_KEYS.TENANT, JSON.stringify(res.tenant));
			}
			localStorage.setItem(YOUKAN_KEYS.ACCOUNT_TYPE, accountType);
	...
	const register = async (name: string, email: string, pass: string, type: 'user' | 'proprietor' | 'company' = 'user', companyName?: string, personalEmail?: string) => {
		...
		try {
			...
			if (data.token) {
				...
				localStorage.setItem(YOUKAN_KEYS.USER, JSON.stringify(data.user));
				localStorage.setItem(YOUKAN_KEYS.TENANT, JSON.stringify(data.tenant));
