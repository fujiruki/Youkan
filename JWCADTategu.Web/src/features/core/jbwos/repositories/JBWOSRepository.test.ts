...
import { JBWOSRepository } from './JBWOSRepository';
...
describe('JBWOSRepository', () => {
    ...
    describe('getItemsByStatus', () => {
        it('should return items from API when API call succeeds', async () => {
            ...
            const result = await JBWOSRepository.getItemsByStatus('inbox');
    ...
    describe('addItemToInbox', () => {
        ...
        it('should not throw if ApiClient.createItem fails', async () => {
            ...
            const id = await JBWOSRepository.addItemToInbox('Fail Task');
