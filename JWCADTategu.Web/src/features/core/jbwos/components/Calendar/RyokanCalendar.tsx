...
export const RyokanCalendar: React.FC<RyokanCalendarProps> = ({
    ...
    layoutMode = 'panorama', displayMode = 'grid', filterMode = 'all',
            ...
            signs.forEach(item => {
                ...
                if (chip) {
                    ...
                    newConnections.push({
                        ...
                        color: item.status === 'focus' ? '#3b82f6' : '#94a3b8'
                    });
                            ...
                            {selectedSigns.map((item: Item) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className="group p-4 bg-slate-50 dark:bg-slate-800/40 hover:bg-white dark:hover:bg-slate-800 border-2 border-transparent hover:border-indigo-500/20 rounded-2xl transition-all cursor-pointer relative overflow-hidden"
                                    >
                                        <div className="flex items-start justify-between relative z-10">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-1">
                                                    {proj && <span className="text-[10px] font-black px-2 py-0.5 bg-indigo-500/10 text-indigo-500 rounded-full">{proj.name}</span>}
                                                    <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">{item.status}</span>
                                                </div>
                                                <h4 className="font-bold text-slate-700 dark:text-slate-200 truncate group-hover:text-indigo-600 transition-colors">{item.title}</h4>
                                            </div>
                                            <div className="text-right ml-4">
                                                <div className="text-xs font-black text-slate-900 dark:text-slate-100">{item.estimatedMinutes ? Math.round(item.estimatedMinutes / 60 * 10) / 10 : 0}h</div>
                                                <div className="text-[10px] text-slate-400 font-bold mt-1">{item.due_date ? format(new Date(item.due_date), 'MM/dd') : 'No Date'}</div>
                                            </div>
                                        </div>
                                    </div>
                                );
