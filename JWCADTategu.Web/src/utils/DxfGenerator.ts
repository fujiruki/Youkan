...
export class DxfGenerator {
    ...
    private header() {
        ...
        // Add build timestamp as comment (999 = comment code)
        this.add(999, `Generated by JWCAD Tategu Web | Build: ${BUILD_TIME_DISPLAY}`);
        this.add(0, 'ENDSEC');
...
export const generateDoorDxf = (
    ...
    let commonScale = 1;
    if (effectiveOptions.useA3Layout && doors.length > 0) {
        const maxDoorWidth = CELL_WIDTH * 0.75; // 97.5mm (25% margin)
        const maxDoorHeight = DRAWING_AREA_HEIGHT * 0.75; // 60mm (25% margin)

        // Find the largest door dimensions across all doors
        let maxWidth = 0;
        let maxHeight = 0;

        doors.forEach(door => {
            const w = door.dimensions.width;
            const h = door.dimensions.height;
            if (w > maxWidth) maxWidth = w;
            if (h > maxHeight) maxHeight = h;
        });

        // Calculate common scale based on the largest door
        const scaleX = maxDoorWidth / maxWidth;
        const scaleY = maxDoorHeight / maxHeight;
        commonScale = Math.min(scaleX, scaleY);

        debugDxf('A3 Common Scale Calculated', {
            maxWidth,
            maxHeight,
            maxDoorWidth,
            maxDoorHeight,
